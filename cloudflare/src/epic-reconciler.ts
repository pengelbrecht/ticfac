...
import { RunStateStore, ROLES, provenance, terminalState, type Checkpoint, type TickState } from "./run-state-store";
...
type DecisionKey = { decision: number; role: string; request: Record<string, unknown>; response: Record<string, unknown> };
        ...
        if (reportIsShaped(report)) {
          ...
          await store.recordDecision({
            ...
            request: {
              ...
              attempt,
              job_id: typeof marker.job_handle.job_id === "string" ? marker.job_handle.job_id : null,
              write_ref: writeRefFor(store.runID, tickID, attempt),
              role,
            },
          ...
          decisions.push({
            decision: number,
            role,
            request: { tick_id: tickID, epic_id: store.epicID, attempt, write_ref: writeRefFor(store.runID, tickID, attempt), role },
            response: { outcome: report.outcome, commits: report.commits, detail: report.detail },
          });
        }
      }
