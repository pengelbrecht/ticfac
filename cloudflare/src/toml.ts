...
export function parseToml(source: string): Table {
  ...
  function parseString(): string {
    ...
    for (;;) {
      ...
      if (triple && src.slice(pos, pos + 3) === quote.repeat(3)) {
        pos += 3;
        return out;
      }
  ...
  function parseValue(): unknown {
    ...
    if (/^[0-9]{4}-[0-9]{2}-[0-9]{2}/.test(token) || /^[0-9]{2}:[0-9]{2}/.test(token)) {
      return fail(
        `date and time values are not supported in .tick/runners.toml: ${JSON.stringify(token)}`,
        start,
      );
    }
    ...
    for (;;) {
      ...
      for (const part of parts.slice(0, -1)) {
        ...
        if (next === undefined) target[part] = newTable();
          ...
          fail(`cannot extend ${JSON.stringify(part)}: it is not a table`);
        target = target[part] as Table;
      }
      const key = parts[parts.length - 1];
      if (Object.hasOwn(target, key)) fail(`duplicate key ${JSON.stringify(parts.join("."))}`);
      target[key] = value;
      skipBlank();
      if (src[pos] === ",") {
        pos++;
  ...
  for (;;) {
    ...
    if (src[pos] === "[") {
      ...
      pos++;
      if (src[pos] === "[")
        fail("array-of-tables [[...]] is not supported in .tick/runners.toml", headerStart);
      const parts = parseKeyPath();
      skipSpaces();
      ...
      if (closed.has(dotted) || assigned.has(dotted))
        fail(`duplicate table [${dotted}]`, headerStart);
    ...
    const value = parseValue();
    const target =
      parts.length > 1 ? descend(current, parts.slice(0, -1), currentPath, false) : current;
    const key = parts[parts.length - 1];
    const dotted = [...currentPath, ...parts].join(".");
    if (Object.hasOwn(target, key)) fail(`duplicate key ${JSON.stringify(dotted)}`);
    target[key] = value;
    assigned.add(dotted);
