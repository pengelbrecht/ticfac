 ...
 *
 * ONE STREAM PER (RUN, TICK), never the orchestrator's single key: a run may
 * hold several worker containers concurrently, and one shared stream would
 * interleave them into nonsense. It lives under the
 ...
 * reason a dispatch does not run.
 */
 ...
 *
 * This is what makes `--tick` discoverable: an operator reading a failed run
 ...
 * id no board or branch has ever heard of.
 */
...
/**
 * The sink a live dispatch streams a worker container's output through.
 *
 * The sequence counter is per tick and lives in this closure, SHARED by every
 * writer `forTick` hands out: a spawn and its wait each bind their own writer
 * for the same container, and two counters would mean two writers PUTting
 * `000001.log` over each other. One epoch per sink, taken once at
 * construction, so everything one dispatch writes for one container sorts
 * together and after whatever an earlier writer left.
 */
