...
test("the bundle in the tree resolves every shared schema_id to one definition", () => {
  ...
  const evidence = uses.get("ticfac.evidence.v3") ?? [];
  const files = new Set(evidence.map((use) => use.file));
  assert.ok(
    files.size >= 2,
    "ticfac.evidence.v3 appears in one contract — job-protocol.json defines it and " +
  ...
  assert.equal(
    ...
    1,
    "ticfac.evidence.v3 must be defined exactly once",
  );
});
...
test("a second definition of a shared schema_id fails — the 1.2.0 shape", () => {
  ...
  assert.throws(
    () => verifySchemaIds(dir),
    /ticfac\.evidence\.v3 is defined 2 times/,
