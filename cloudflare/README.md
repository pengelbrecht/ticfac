...
| `test/fixtures/tk-write-corpus.json` | The pinned corpus of tk's own write bytes, GENERATED by the Go package `internal/tkcorpus` (which also regenerates and guards it: `go test ./internal/tkcorpus -update` from the repo root). `test/tk-write-corpus.test.ts` replays every case through `src/tracker-client.ts` and byte-compares the committed records against it — the client's writes must be tk's writes, and neither side can drift quietly. Never hand-edit it. |
