...
| `src/run-done.ts` | The completion door (tick 7eq): a finished orchestrator's POST, turned into `instance.sendEvent()` so its Run Workflow wakes without polling. Best effort by design — the pushed branch, never the event, is the source of truth. Authorized by the run's gateway token like `/api/wave`. |
...
| `src/sandbox-executor.ts` | The sandbox compatibility executor (Phase 4 item 4): the per-tick Sandbox dispatch machinery — `AttemptSpec`'s four operations (start, inspect, collect, cancel, `src/attempt-protocol.ts`) over the `SANDBOXES` binding — reached through the per-tick dispatch door rather than in-isolate: the Go orchestrator in the container cannot create a sibling Sandbox itself, so it calls the door over HTTP and this module is the Worker side of that call. |
| `src/sandbox-dispatch.ts` | The per-tick sandbox dispatch door (tick 8ty): the HTTP surface the orchestrator CONTAINER calls, because a container cannot create a sibling Sandbox itself — start one attempt's worker container by identity (adopting a live one rather than booting a rival) and read one back as running, finished or absent. Authorized by the run's gateway token like `/api/wave`; the contract the Go client and this route both build to is documented here, in one place. |
