...
Two smaller defects also came out of the run and were fixed in this epic: the shipped binary could not locate the tk JSON manifest outside the ticfac checkout (u65, embedded manifest), and a worker committed its RESULT artifact into the attempt branch (wtd, in progress at the time of writing — `runs/gate-cia-2/uqe/RESULT-uqe.md` is on the ticks PR).
