...
// StateFile is the name of the per-run notifier state, written beside the
// run's attempt records in the executor's state root.
//
// The leading dot is LOAD-BEARING, not a style choice, and inherited from the
// tk design: every enumeration of a state directory that picks up `*.json`
// must skip this file or it becomes a phantom worker with an empty tick id —
// observed live in ticks before the dot was added. The executor's own
// directory walk (findAttemptState) stops at the first attempt.json, but any
// future enumeration must keep skipping dot-files.
...
// StatePath is where a run's notifier state lives: inside the run's directory
// under the executor's state root, never inside a repository.
func StatePath(stateRoot, runID string) string {
	return filepath.Join(stateRoot, runID, StateFile)
}
...
// already told them about THIS worker" and "has anything new joined the
// wave", and both are set membership.
type State struct {
	...
	// tick that is NOT in here — so a fresh attempt re-arms the chime and a
	// teardown does not.
...
// LoadState reads a run's notifier state. A missing, unreadable, malformed
...
// SaveState writes a run's notifier state atomically, creating the run's
// state directory if the executor has not made it yet.
...
// Lock takes an exclusive lock for one run's state file and returns the
// release function.
//
...
// than flock because the state lives on whatever filesystem the state root
// happens to be on.
//
