...
This is ticfac's copy of the reference, lifted from ticks' skill doc (`skills/ticks/references/herdr-kinds.md`) at the av8 base, with the **pi** section added from the live verification run of 2026-09-10 (tick gjk — pi 0.85.1, herdr protocol 22 (0.9.0); evidence: `.tick/logs/herd/av8/gjk.RESULT.md`) and every stale pi statement the old copy carried corrected to what that run observed. ticks' copy remains the operator-facing skill doc until tick 4l2 retires ticks' execution machinery; until then the claude/codex/opencode sections here are transcriptions of their original live round trips (2026-08-12/13), not re-derivations.
...
Four of those — **claude**, **codex**, **opencode** and **pi** — are verified below as tick implementers (pi on 2026-09-10, tick gjk). The rest are reachable through the same three-step recipe in [Adding a kind](#adding-a-kind); do not assume a template for a kind nobody has round-tripped.
...
| `pi` | `--model <provider>/<model>` | `--approve` (see [pi](#pi)) | **current (v8)** — `~/.pi/agent/extensions/herdr-agent-state.ts` | `path` — a `~/.pi/agent/sessions/…jsonl` file path, **not a UUID** | `pi --session <uuid>` |
...
The compilation below, its fixed argv order, and the fail-closed refusal further down are implemented by this package (`kinds.go` / `compile.go` — `tk herd spawn` in ticks), and the argv herdr echoes on `agent_started` is the ground truth for what actually landed: read it back rather than trusting the template.
...
All four rows are round-tripped live (see their sections below) — `claude`/`codex`/`opencode` on 2026-08-12/13, `pi` on 2026-09-10 (tick gjk).
...
| `pi` | Cross-provider; a **provider-qualified** id spelled exactly as `pi --list-models` prints it — the spawner validates the id against that catalog before a pane exists | `openai-codex/gpt-5.6-sol`, `anthropic/claude-opus-4-6`, `cloudflare-workers-ai/@cf/zai-org/glm-5.3` | bare ids that name no provider, and ids the local catalog does not list — which would otherwise cost a turn that returns **empty at exit 0**, no error anywhere on stdout |

Effort levels are sparse too: `claude --effort` accepts `low, medium, high, xhigh, max` (no `off`/`minimal`); `pi --thinking` accepts `off, minimal, low, medium, high, xhigh, max` — but the dimension itself is **per model**: pi's catalog carries a `thinking` column, several entries read `no`, and the CLI silently accepts and ignores a level on those (verified live 2026-09-10, tick gjk), so the spawner refuses the combination at compile time rather than let a tier mean something it does not; codex's `model_reasoning_effort` is a config string whose accepted set is the model's, not the CLI's (`max` is live on this machine's `~/.codex/config.toml`); `opencode` accepts **no level at all** on the interactive CLI, so every level is refused.
...
**Verified template** *(pi 0.85.1, herdr protocol 22 (0.9.0), verified live 2026-09-10, tick gjk — every command and its output in `.tick/logs/herd/av8/gjk.RESULT.md`)*.

```bash
herdr agent start <name> --kind pi --pane <pane-id> --timeout 120000 \
  -- --approve --model <provider>/<model>[:<effort>]
```

- **There is no permission gate to bypass.** `pi --help` has no permission-mode, skip-permissions or approval-bypass flag; the only approval-shaped flags are `--approve, -a` / `--no-approve, -na`, and they govern trust of project-local files (AGENTS.md / CLAUDE.md discovery), not tool use. Tested directly, in a scratch git repo OUTSIDE the trusted tree, with no approval flag at all: `pi -p --model <id> "Create a file named proof.txt containing exactly the word VERIFIED. Use your write tool. Then reply DONE."` wrote the file and replied `DONE`, exit 0. **pi ran a write tool with no prompt and no flag**, so a missing full-auto template for this kind was a category error, not a missing template. `~/.pi/agent/trust.json` records the trusted roots, and `--approve` is what makes an *untrusted* path non-interactive — which is why it belongs in the template even though the probe above passed without it.
- **Model.** `--model <pattern>` takes a `provider/id`, with an optional `:<thinking>` suffix: `--model cloudflare-workers-ai/@cf/zai-org/glm-5.3-flash:high`. Effort is a **suffix** on the model id (or `--thinking <level>` when no model is named); the accepted set is `off, minimal, low, medium, high, xhigh, max`.
- **`pi --list-models` is the model oracle.** It prints provider, model, context, max-out, **thinking** and images (84 rows on the machine verified), filters with `pi --list-models <search>`, and answers `No models matching "<x>"` for an unknown id. `pi auth check --model <nonsense>` is NOT an oracle: it validates the provider only and reports ready. The spawner validates a pi tier's model id against the catalog before a pane exists, and the **thinking column is per model** — several cloudflare entries read `no` — so an effort level on such a model is a compile-time refusal naming the tier, the model and the column (the level is otherwise silently accepted and ignored, verified live).
- **A bad model id is a silent empty turn.** Not opencode's substitution and not a clean failure, but a failure mode of its own: `--model cloudflare-workers-ai/@cf/zai-org/glm-9-imaginary` warns on stderr (`Warning: Model "…" not found for provider "cloudflare-workers-ai". Using custom model id.`), passes the id THROUGH to the provider, returns an **empty turn** and **exits 0**. Any automation reading exit codes believes it worked. The read-back that catches it: **pi's own status line names the model and effort actually in use** — `(cloudflare-workers-ai) @cf/zai-org/glm-5.3-flash • high`. The catalog check above is what makes the first-round-trip gate unnecessary for this class: the typo dies at compile time.
- **`agent_session` is a `path`, not a UUID.** `~/.pi/agent/sessions/<project-slug>/<timestamp>_<uuid>.jsonl` — a transcript file. pi's exit message prints the resume form with the short UUID, not the path.
...
```text
$ herdr agent start gjk-pi --kind pi --pane wPH:p1 --timeout 120000 \
    -- --approve --model cloudflare-workers-ai/@cf/zai-org/glm-5.3-flash:high
{"result":{"agent":{"agent_status":"idle","interactive_ready":true}, …},
 "argv":["pi","--approve","--model",
         "cloudflare-workers-ai/@cf/zai-org/glm-5.3-flash:high"]}

$ herdr agent prompt gjk-pi "Reply with the single word BANANA" --wait --timeout 120000
BANANA        agent_status: done        cost, from pi's own status line: $0.002

agent_session: {"kind":"path","value":"~/.pi/agent/sessions/<slug>/
  2026-09-10T09-45-45-039Z_01a08ab5-7d4f-70ca-8472-20398f822c94.jsonl"}

$ herdr integration status
pi: current (v8) (~/.pi/agent/extensions/herdr-agent-state.ts)
```

The echoed argv is the proof the template landed; the status line is the proof the model did. pi IS on the interactive shell's PATH (`zsh -i -c 'which pi'` resolves through fnm), so no absolute path is needed in the template.
...
`agent_session.kind` is `id` for claude, codex and opencode; **pi reports `path`** — a `~/.pi/agent/sessions/…jsonl` transcript file path, not a UUID. `value` is the token the resume flag takes for the `id` kinds — a UUID for claude and codex, an opaque `ses_…` string for opencode; pi's resume token is the short UUID its exit message prints, not the path. All four CLIs also print the resume form on exit, which is a useful cross-check that Herdr's view is the real one:
...
| `pi` | `To resume this session: pi --session <uuid>` | `herdr agent start <n> --kind pi --pane <p> -- --approve --model <provider>/<model> --session <uuid>` |

All four were verified live: after `C-c`-ing each agent (twice for claude, codex and **pi** — pi's own exit message says press Ctrl+C twice — and **once** for opencode) and restarting it in the same pane under a fresh name with the resume form, `herdr pane read` showed the prior turn (`Reply with the single word OK` → `OK`, and for pi `BANANA`) replayed in the restored transcript, and for claude the `agent_session.value` was unchanged across the restart.
...
**pi has a third form, verified 2026-09-10 (tick gjk): the id passes through as a "custom model id"** — a stderr warning, an empty turn, and **exit 0**. No substitution (better than opencode: no turn runs on a different model) and no failure either (worse than codex's visible 400). pi ships the defence itself: `pi --list-models` is a real catalog oracle, so the spawner refuses a typo'd pi id at compile time, before a pane exists — the one kind of the four where this trap is closed *statically*, not by the gate. pi's own status line (`(provider) model • effort`) is the read-back when verifying by hand.
