...
// The fake harness for contracts/lifecycle-invariants.json: a model of exactly
// the behaviour SPEC Appendix A's thirteen invariants depend on, and nothing
// else. No git, no container, no network, no clock.
//
...
// one is OBSERVABLE — which is what let the suite exist in ticks before any
// reconciler code did, and what lets ticfac inherit it here, unchanged, before
// the reconciler this repository will hold.
//
// ticks and the factory each implement this independently. This is the third,
// and it is the one the real reconciler and every executor must go on to pass:
// `gate.applies_to` names them.
...
type lifecycleHarness struct {
	...
	// claim is that this, and not a separate heartbeat, keeps a job alive.
	...
	// off turns a named guard off, for the negative control: a guard nothing
	// has ever seen refuse is not known to be a guard.
	off map[string]bool
}

type fakeCredential struct{ revoked bool }
...
func newLifecycleHarness(c lifecycleContract) *lifecycleHarness {
	h := &lifecycleHarness{
	...
	// A10's boundary comes from the fixture, not from this file. Two hard-coded
	// copies of one boundary is two copies that can drift apart with both
	// suites green.
...
func (h *lifecycleHarness) guarded(name string) bool { return !h.off[name] }

func (h *lifecycleHarness) job(name string) *fakeJob {
	j, ok := h.jobs[name]
	if !ok {
		j = &fakeJob{}
		h.jobs[name] = j
...
func (h *lifecycleHarness) run(t *testing.T, s lifecycleStep) string {
	t.Helper()
	switch s.Op {
	...
	case "issue_credential":
		...
		h.credentials[s.Job] = &fakeCredential{}
		return "issued"
	...
	case "claim":
		h.claims[s.Path] = &fakeClaim{actor: s.Actor}
		return "claimed"
	...
	case "strike_out":
		h.holds[s.Unit] = &fakeHold{struck: true}
		return "struck"
	...
	case "set_budget":
		h.budget = fakeBudget{requested: s.Requested, ceiling: s.Ceiling, set: true}
...
func (h *lifecycleHarness) bootedJobs() []string {
	out := []string{}
...
func (h *lifecycleHarness) issuedCredentials() []string {
	out := []string{}
...
func (h *lifecycleHarness) tornDownJobs() []string {
	out := []string{}
	for name, j := range h.jobs {
		if j.tornDown {
			out = append(out, name)
...
func (h *lifecycleHarness) settledClaims() []string {
	out := []string{}
...
func (h *lifecycleHarness) reportedClasses() []string {
	out := []string{}
	for _, r := range h.reports {
...
func (h *lifecycleHarness) evidenceKeys() []string {
	out := []string{}
...
func (h *lifecycleHarness) releasedBy() map[string]string {
...
func finalMismatches(h *lifecycleHarness, want lifecycleFinal) []string {
	var bad []string
...
func finalMatches(h *lifecycleHarness, want lifecycleFinal) bool {
