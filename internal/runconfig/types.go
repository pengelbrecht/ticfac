...
type Role struct {
	...
	// Substrates is the per-substrate overlay (ticks 84z and 5uo), keyed by
	// substrate: the role's cell in the substrate's override file —
	// `.tick/runners.cloud.toml` for the cloud, `.tick/runners.local.toml`
	// for herdr and harness — recorded by [LoadFor] so resolution can apply
	// it LAST, over the role's own values AND any tier. It is never read from
	// runners.toml itself: the inline `[roles.<name>.substrates.<substrate>]`
	// form 84z shipped is refused, pointing at the file (two ways to say one
	...
	// a REFUSAL naming the role ([ErrNoCloudRouting]), never a fall back to
	// the base cell, which is how a cloud run reached a claude process nobody
	// chose.
	Substrates map[string]*TierVariant `toml:"-"`
	// SubstrateTiers is the override file's own `[roles.<name>.tiers.<tier>]`
	// cells, keyed by substrate then tier. They are re-applied after the
	// override's role cell, so an override that routes a whole role (pi on
	// GLM) can still vary it per tier (GLM Flash at economy) without a tier
	// from the COMMON file ever winning over it — the hole 84z left, finding
	// ea1a62d3.
	SubstrateTiers map[string]map[string]*TierVariant `toml:"-"`
	// InlineSubstrates catches the retired inline form so it is refused with
	// the file it moved to, rather than as an anonymous unknown key.
	InlineSubstrates map[string]any `toml:"substrates"`
}
