...
func parsed(t *testing.T, c *contract) (map[string]*contractschema.Schema, map[string]*contractschema.Schema) {
	t.Helper()
	defs := make(map[string]*contractschema.Schema, len(c.Defs))
	for name, raw := range c.Defs {
		schema, err := contractschema.ParseSchema(raw)
		if err != nil {
			t.Fatalf("$defs.%s: %v", name, err)
		}
		defs[name] = schema
	}
	records := make(map[string]*contractschema.Schema, len(c.Records))
	for name, record := range c.Records {
		if len(record.Schema) == 0 {
			t.Fatalf("record %s carries no schema", name)
		}
		schema, err := contractschema.ParseSchema(record.Schema)
		if err != nil {
...
func TestGoldenExamplesValidate(t *testing.T) {
	...
	for _, example := range c.Examples.Golden {
		...
		if errs := contractschema.Validate(schema, defs, decode(t, example.Document)); len(errs) > 0 {
...
func TestNegativeExamplesFail(t *testing.T) {
	...
	for _, example := range c.Examples.Negative {
		...
		errs := contractschema.Validate(schema, defs, decode(t, example.Document))
		if len(errs) == 0 {
...
func TestTheSpecIllustrationIsAGoldenExample(t *testing.T) {
	...
	for _, example := range c.Examples.Golden {
		...
		if errs := contractschema.Validate(records["job_spec"], defs, document); len(errs) > 0 {
...
func resolve(t *testing.T, s *contractschema.Schema, defs map[string]*contractschema.Schema) *contractschema.Schema {
	t.Helper()
	if s == nil {
...
func mustDef(t *testing.T, defs map[string]*contractschema.Schema, name string) *contractschema.Schema {
	t.Helper()
	def, ok := defs[name]
	if !ok {
...
func enumValues(s *contractschema.Schema) []any {
	if s == nil {
		return nil
	}
	return s.Enum
...
func enumStrings(s *contractschema.Schema) []string {
...
func typeNames(s *contractschema.Schema) []string {
	if s == nil {
		return nil
	}
