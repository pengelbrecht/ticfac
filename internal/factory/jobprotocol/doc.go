...
// cloudflare/test/job-protocol.test.ts. A schema, a golden example or a
// refusal changed on one side only fails the other.
//
// It is also where the bundle's ONE evidence record lives (SPEC §10.1,
// ticfac.evidence.v3). contracts/ticfac-run-state.json places the file and
...
//
// The schemas themselves are validated with internal/schema, the strict JSON
// Schema subset this repository carries (ported from the ticks-side validator
// these tests originally ran under): a keyword that subset cannot enforce
// makes the contract fail to parse rather than being quietly ignored, which is
// the difference between a contract and a decorative copy of one
// (contracts/README.md).
package jobprotocol
