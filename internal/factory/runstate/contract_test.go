...
func TestContractIdentifiesItself(t *testing.T) {
	c := load(t)
	// Bundle 5.1.0 (tick to1) bumped ticfac.run_state to schema_version 3 for
	// the substrate fields in provenance. This test moved in from ticks (tick
	// ek7) pinned at 1 and has followed every bundle since.
...
func parseSchemas(t *testing.T, c runStateContract) (map[string]*contractschema.Schema, map[string]*contractschema.Schema) {
	t.Helper()
	defs := map[string]*contractschema.Schema{}
	for name, raw := range c.Defs {
		s, err := contractschema.ParseSchema(raw)
		if err != nil {
			t.Fatalf("$defs.%s: %v", name, err)
		}
		defs[name] = s
	}
	schemas := map[string]*contractschema.Schema{}
	for name, raw := range c.Schemas {
		s, err := contractschema.ParseSchema(raw)
		if err != nil {
...
func TestGoldenExamplesValidate(t *testing.T) {
	...
	for record, raw := range c.Golden {
		...
		if errs := contractschema.Validate(s, validatorDefs, doc); len(errs) > 0 {
...
func TestInvalidExamplesAreRefused(t *testing.T) {
	...
	for i, bad := range c.Invalid {
		...
		errs := contractschema.Validate(s, validatorDefs, any(bad.Document))
		if len(errs) == 0 {
