...
func referencedSchema(t *testing.T, c runStateContract, record string) (*contractschema.Schema, map[string]*contractschema.Schema) {
	t.Helper()
	...
	schema, err := contractschema.ParseSchema(entry.Schema)
	if err != nil {
...
func TestGoldenEvidenceValidatesAgainstTheReferencedDefinition(t *testing.T) {
	...
	if errs := contractschema.Validate(schema, defs, doc); len(errs) > 0 {
