...
// worker's. The record was produced by cloudflare/src/auth.ts itself
...
func TestDeriveTokenHashMatchesGoldenVector(t *testing.T) {
	...
	if got != goldenHash {
		t.Errorf("hash record mismatch with cloudflare/src/auth.ts:\n got %s\nwant %s", got, goldenHash)
	}
}
...
// in cloudflare/test/auth.test.ts. Cloudflare Workers refuses PBKDF2 above
...
func TestGoldenVectorMatchesWorker(t *testing.T) {
	...
	script := `
import { deriveTokenHash } from "../../cloudflare/src/auth.ts";
...
`
	...
	if got := strings.TrimSpace(string(out)); got != goldenHash {
		t.Errorf("cloudflare/src/auth.ts derives %s, this package pins %s", got, goldenHash)
	}
}
