...
// (write-only) and ~/.ticfacrc (0600), so `ticfac factory status` can re-check it.
...
const (
	...
	// traffic, but its cost budget has nothing to act on, which `ticfac factory
...
// same code path serves `ticfac factory setup` typed by a human and one driven by
// flags in a script.
type SetupOptions struct {
	// Version is the ticfac version a deploy started from here records against.
	Version string
...
func Setup(ctx context.Context, opts SetupOptions) (*SetupResult, error) {
	...
	fmt.Fprintf(out, "  check them any time with `ticfac factory status`\n")
	return result, nil
}
...
func setupDeployment(
	...
	if url == "" {
		...
		if !isYesDefaultYes(answer) {
			return errors.New("no factory is deployed — run `ticfac factory deploy` (or answer yes here) before configuring credentials")
		}
...
func storeGitHubCredential(
	...
	if cred.Auth == AuthDeviceFlow && !cred.ExpiresAt.IsZero() {
		...
		fmt.Fprintf(out, "  the credential %s; `ticfac factory setup` renews it without a browser\n",
...
func repoScopeRemedy(auth, repo string) string {
	if auth == AuthDeviceFlow {
		return fmt.Sprintf("The ticks App is not installed on %s, or you did not select it when you "+
			"approved the code. Run `ticfac factory setup` again and tick that repository.", repo)
	}
...
func setupCostTelemetry(
	...
	if token == "" {
		...
		fmt.Fprintf(out, "Add one with: ticfac factory setup --cloudflare-api-token <token>\n")
...
//
// This is a COPY of the ticks repository's internal/github
// DetectProject/ParseProjectFromRemote, not an import of them — the same
// decision ticks made, kept here: internal/github is the tracker's GitHub
// client, and its main business is the OAuth device flow: token exchange,
// credential storage, the App's own identity. Promoting it to reach this one small remote
