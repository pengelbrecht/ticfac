...
func TestCredentialOwnershipContractHasAValidatingExample(t *testing.T) {
	...
	if errs := contractschema.Validate(schema, nil, example); len(errs) > 0 {
...
// cloudflare/test/json-schema.ts, which refuses the same documents with the
...
func parseCredentialSchema(t *testing.T, contract credentialContract) *contractschema.Schema {
	t.Helper()
	schema, err := contractschema.ParseSchema(contract.Schema)
	if err != nil {
...
// about. `cloudflare/test/json-schema.ts` matches Go's message text
...
func TestCredentialSchemaRefusesItsNegativeExamples(t *testing.T) {
	...
	for i, bad := range contract.Invalid {
		...
		errs := contractschema.Validate(schema, nil, any(bad.Document))
		if len(errs) == 0 {
