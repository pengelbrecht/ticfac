...
// The image builds its tk from the PINNED ticks ref (factory.pin.json, see
// sourceref.go), so the pins wrangler builds with are the pin's answers —
// not whatever the committed Dockerfile happened to say last release. That
// is the whole mechanism keeping the container's tk in step with the
// entrypoint it boots, and it is what makes a deploy reproducible from a
// reviewable diff.
func TestDeployPinsTheImagesTkToThePinnedSource(t *testing.T) {
	h := newHarness(t)
	...
	if result.SourceRef != pin.Ref {
		t.Errorf("Result.SourceRef = %q, want the pinned ref %q", result.SourceRef, pin.Ref)
	}
	...
	for _, want := range []string{
		"\nARG TK_VERSION=" + pin.Version + "\n",
		"\nARG TK_SOURCE_REF=" + pin.Ref + "\n",
		"\nARG TK_MODULE=" + pin.Module + "\n",
	} {
...
// The committed Dockerfile's TK_MODULE default and the pin's module have to
// name the same Go module: the deploy rewrites the VERSION and SOURCE pins in
// its staged copy but leaves the module line alone, so a pin that names a
// module the image would never install is a mismatch this catches at test
// time rather than mid-docker-build.
func TestThePinnedModuleIsTheModuleTheImageInstalls(t *testing.T) {
	pin, err := PinnedSource()
	if err != nil {
		t.Fatalf("PinnedSource: %v", err)
	}
	data, err := ReadSandboxFile("Dockerfile")
	if err != nil {
		t.Fatalf("reading the embedded Dockerfile: %v", err)
	}
	if !strings.Contains(string(data), "\nARG TK_MODULE="+pin.Module+"\n") {
		t.Errorf("the embedded Dockerfile's ARG TK_MODULE does not match %s's module %q — the\nimage installs a different tk than the one the pin names:\n%s",
			SourcePinFile, pin.Module, data)
	}
}

// The failure this replaces — a container that booted, streamed, and died at
// exit 6 with "unknown command: sandbox" — is now caught by the Dockerfile's
// own assertion layer, which runs every line of required-tk-commands against
// the tk the image ACTUALLY built from the pinned ref. The fast preflight
// against the deploying binary was dropped with the move (see the header of
// tkcommands.go): the tests that covered it in ticks
// (TestDeployStopsWhenThisTkCannotRunTheEntrypoint,
// TestDeployRefusesWithoutACommandSet) moved with it, and
// TestRequiredTkCommandsFileMatchesTheEntrypoint in tkcommands_test.go keeps
