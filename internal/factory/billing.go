...
// defaultCloudflareAPIBase is Cloudflare's REST root. It moved here from
// ticks' setup.go with this file: billing.go is the only mover in this tick
// that reads it, and when setup.go itself moves (ticks tick b3a) it must take
// THIS copy rather than ship a second one.
const defaultCloudflareAPIBase = "https://api.cloudflare.com/client/v4"

// gatewayIDs reads the Cloudflare account and gateway names out of an AI
// Gateway base URL — https://<host>/v1/<account>/<gateway>[/...]. One
// configured value names the gateway everywhere (moved from ticks' setup.go
// with this file for the same reason as defaultCloudflareAPIBase).
