...
// legacyFileName is ~/.ticksrc's name — the pre-split file ticks' board sync
// still owns (internal/tickboard/cloud/client.go in the ticks repository).
// It is spelled here rather than imported, because this migration is the one
// place factory code is allowed to know that name at all: ticfac never reads
// a value out of the file, it only moves factory-owned lines out of it.
...
// after upgrading: `ticfac factory setup`, `ticfac factory deploy`, `ticfac
// factory status`, `ticfac cloud …`, `ticfac factory dashboard`. Commands
// with no factory business must never call this — that is the whole point of
// the split (see internal/factory/credentials).
