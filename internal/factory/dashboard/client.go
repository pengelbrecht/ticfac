...
// mirrors ACTIVE_RUN_STATES in cloudflare/src/runs.ts.
...
// the fencing token is never served (see cloudflare/src/run-room.ts).
type Lease struct {
	RunID       string `json:"run_id"`
	Epic        string `json:"epic"`
...
// `PendingEntry` in cloudflare/src/run-room.ts.
//
