...
// NOT moved from ticks with the rest of migrate_test.go: the fixture that
// walks a migrated ~/.ticfacrc through a live `factory status` call
// (TestMigratedCredentialsStillAuthenticateAgainstStatus) — status.go is the
// read path and moves with ticks tick 0e1 ("Factory move C"), so that test
// belongs there, where Status and StatusOptions are.
