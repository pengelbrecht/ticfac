...
// resumableCurlFlags make a download survive a dropped connection.
//
...
//	-C -                    RESUME. This is the one that matters: without it
//	                        each retry restarts from zero, and a 116 MB file
//	                        on a link that drops every 40 MB never finishes
//	                        however many times it is retried.
const resumableCurlFlags = "--retry 10 --retry-all-errors --retry-delay 2 -C -"
