	...
	if _, err := w.run(ctx, "", "whoami"); err != nil {
		return &PrerequisiteError{
			...
			Detail: "Run `wrangler login` (or set CLOUDFLARE_API_TOKEN) so wrangler can reach\n" +
				"your Cloudflare account, then run `ticfac factory deploy` again.",
			Err: err,
		}
	}
	return nil
...
func (w *wrangler) ensureDatabase(ctx context.Context, name string) (id string, created bool, err error) {
	...
	if id == "" {
		return "", true, fmt.Errorf("created D1 database %q but it is not in `wrangler d1 list`; re-run `ticfac factory deploy`", name)
	}
	return id, true, nil
}
