...
// The command tests for `ticfac cloud logs`, ported from ticks'
// cmd/tk/cmd/cloud_logs_test.go: the harness is the one in cloud_test.go
// (newCloudFactory, configureCloudFactory, setupCloudRepo, runCloudArgs), and
// ticks' ExecuteArgs-with-captured-output becomes Run(args, stdout, stderr)
// with the exit code asserted. configureCloudflareAPI and erroredInstance
// come from cloud_supervisor_test.go, as they did in ticks.
...
func TestCloudLogsPrintsTheHarnessOutput(t *testing.T) {
	...
	code, out, stderr := runCloudArgs(t, []string{"cloud", "logs", "run_live"})
	if code != exitSuccess {
		t.Fatalf("cloud logs: %s\n%s", stderr.String(), out.String())
	}
...
func TestCloudLogsSaysWhenTheReadWasBounded(t *testing.T) {
	...
	code, out, stderr := runCloudArgs(t, []string{"cloud", "logs", "run_big"})
	if code != exitSuccess {
		t.Fatalf("cloud logs: %s", stderr.String())
	}
	output := out.String()
...
func TestCloudLogsTailKeepsTheLastLines(t *testing.T) {
	...
	code, out, stderr := runCloudArgs(t, []string{"cloud", "logs", "run_live", "--tail", "2"})
	if code != exitSuccess {
		t.Fatalf("cloud logs --tail: %s", stderr.String())
	}
	output := out.String()
...
func TestCloudLogsReportsAnEmptyStreamWithTheRunState(t *testing.T) {
	...
	code, out, stderr := runCloudArgs(t, []string{"cloud", "logs", "run_quiet"})
	if code != exitSuccess {
		t.Fatalf("cloud logs: %s", stderr.String())
	}
...
func TestCloudLogsSurfacesAnUnknownRun(t *testing.T) {
	...
	configureCloudFactory(t, endpoint)
	code, _, stderr := runCloudArgs(t, []string{"cloud", "logs", "run_ghost"})
	if code == exitSuccess || !strings.Contains(stderr.String(), "no run run_ghost") {
		t.Fatalf("an unknown run is not reported: %s", stderr.String())
	}
}
...
func TestCloudLogsTickPrintsOneWorkersOwnOutput(t *testing.T) {
	...
	code, out, stderr := runCloudArgs(t, []string{"cloud", "logs", "run_wave", "--tick", "0fg"})
	if code != exitSuccess {
		t.Fatalf("cloud logs --tick: %s\n%s", stderr.String(), out.String())
	}
	if !strings.Contains(out.String(), "FATAL POST /v1/chat/completions -> 404") {
...
func TestCloudLogsNamesTheWorkerStreamsAboveTheLog(t *testing.T) {
	...
	code, out, stderr := runCloudArgs(t, []string{"cloud", "logs", "run_wave"})
	if code != exitSuccess {
		t.Fatalf("cloud logs: %s", stderr.String())
	}
	output := out.String()
...
func TestCloudLogsTellsAMissingStreamFromAnEmptyOne(t *testing.T) {
	...
	code, out, stderr := runCloudArgs(t, []string{"cloud", "logs", "run_wave", "--tick", "zzz"})
	if code != exitSuccess {
		t.Fatalf("cloud logs --tick zzz: %s", stderr.String())
	}
	missing := out.String()
	...
	code, out, stderr = runCloudArgs(t, []string{"cloud", "logs", "run_wave", "--tick", "0fg"})
	if code != exitSuccess {
		t.Fatalf("cloud logs --tick 0fg: %s", stderr.String())
	}
	if !strings.Contains(out.String(), "nothing in it") {
...
func TestCloudLogsRefusesAnEmptyTick(t *testing.T) {
	...
	configureCloudFactory(t, endpoint)
	code, _, stderr := runCloudArgs(t, []string{"cloud", "logs", "run_wave", "--tick", "  "})
	if code == exitSuccess || !strings.Contains(stderr.String(), "--tick takes a tick id") {
		t.Fatalf("an empty --tick is not refused: %s", stderr.String())
	}
	if len(*requests) != 0 {
...
func TestCloudLogsStatesTheTraceIDEvenWithNoOutput(t *testing.T) {
	...
	if !strings.Contains(out.String(), "tr_00000000000000000000000000000abc") {
...
func TestCloudLogsSaysNothingWhenTheRunHasNoTraceID(t *testing.T) {
	...
	code, out, stderr := runCloudArgs(t, []string{"cloud", "logs", "run_old"})
	if code != exitSuccess {
		t.Fatalf("cloud logs: %s", stderr.String())
	}
...
func TestCloudLogsFollowPrintsOnlyWhatIsNew(t *testing.T) {
	...
	code, out, stderr := runCloudArgs(t, []string{"cloud", "logs", "run_live", "--follow", "--interval", "10ms"})
	if code != exitSuccess {
		t.Fatalf("cloud logs --follow: %s\n%s", stderr.String(), out.String())
	}
	output := out.String()
...
func TestCloudLogsFollowStatesTheBytesItMissed(t *testing.T) {
	...
	code, out, stderr := runCloudArgs(t, []string{"cloud", "logs", "run_fast", "--follow", "--interval", "10ms"})
	if code != exitSuccess {
		t.Fatalf("cloud logs --follow: %s\n%s", stderr.String(), out.String())
	}
	output := out.String()
...
func TestCloudLogsFollowStopsWhenTheSupervisorIsDead(t *testing.T) {
	...
	code, out, stderr := runCloudArgs(t, []string{"cloud", "logs", "run_2xm", "--follow", "--interval", "10ms"})
	if code != exitSuccess {
		t.Fatalf("cloud logs --follow: %s\n%s", stderr.String(), out.String())
	}
	output := out.String()
...
func TestCloudLogsFollowWithoutTheCloudflareTokenStillFollows(t *testing.T) {
	...
	code, out, stderr := runCloudArgs(t, []string{"cloud", "logs", "run_quiet", "--follow", "--interval", "10ms"})
	if code != exitSuccess {
		t.Fatalf("cloud logs --follow: %s\n%s", stderr.String(), out.String())
	}
	output := out.String()
...
func TestCloudLogsFollowRefusesANonPositiveInterval(t *testing.T) {
	...
	configureCloudFactory(t, endpoint)
	code, _, stderr := runCloudArgs(t, []string{"cloud", "logs", "run_live", "--follow", "--interval", "0s"})
	if code == exitSuccess || !strings.Contains(stderr.String(), "--interval") {
		t.Fatalf("a zero interval was accepted: %s", stderr.String())
	}
}
