...
// the number in ticfac/cloudflare/src/workflow-limits.ts. It is matched on the
...
func cloudStepCapHint(supervisor *factory.Supervisor) string {
	...
	return "  note: 600000ms is Cloudflare's per-step EXECUTION cap (ten minutes), and it fails the whole\n" +
		...
		"    bounded legs — see ticfac/cloudflare/src/workflow-limits.ts and repo-wiki/debugging-a-live-cloud-run.md."
}
