...
// --tick-ids is the CLI's route to the per-tick-container path (tick pjq).
func TestCloudRunTickIDsCarryTheWaveIntoTheSubmission(t *testing.T) {
	stubCloudTk(t)
	setupCloudWaveRepo(t, "aaa", "bbb")
	endpoint, requests := newCloudFactory(t, func(request cloudFactoryRequest) (int, any) {
		if request.Method != http.MethodPost || request.Path != "/api/runs" {
			return http.StatusNotFound, map[string]any{"error": "not_found"}
		}
	...
	code, out, stderr := runCloudArgs(t, []string{"cloud", "run", "epic1", "--tick-ids", "aaa,bbb"})
	if code != exitSuccess {
		t.Fatalf("cloud run --tick-ids: %s\n%s", stderr.String(), out.String())
	}
	if len(*requests) != 1 {
		t.Fatalf("factory received %d request(s), want one", len(*requests))
	}
	...
	if output := out.String(); !strings.Contains(output, "run_wave") ||
		!strings.Contains(output, "aaa") || !strings.Contains(output, "bbb") {
		t.Errorf("output does not report the dispatched wave:\n%s", output)
	}
}

// A plain `ticfac cloud run` must stay a Phase 1 submission: no empty wave
// field that the factory would have to interpret.
...
func TestCloudRunRefusesTickIDsWithQueue(t *testing.T) {
	stubCloudTk(t)
	repo, _, sha := setupCloudWaveRepo(t, "aaa")
	var calls int
	endpoint, _ := newCloudFactory(t, func(cloudFactoryRequest) (int, any) {
		calls++
		return http.StatusCreated, map[string]any{"run": map[string]any{"run_id": "run_should_not_start"}}
	...
	code, _, stderr := runCloudArgs(t, []string{"cloud", "run", "epic1", "--tick-ids", "aaa", "--queue"})
	if code == exitSuccess {
	...
	for _, want := range []string{"--tick-ids", "--queue"} {
		if !strings.Contains(stderr.String(), want) {
			t.Errorf("refusal %q does not name %s", stderr.String(), want)
		}
	}
	if calls != 0 {
...
func TestCloudRunRefusesAnUnusableWaveBeforeSubmitting(t *testing.T) {
	...
	for name, tc := range map[string]struct {
		...
		if !strings.Contains(stderr.String(), tc.want) {
			t.Errorf("%s: refusal %q does not mention %q", name, stderr.String(), tc.want)
		}
	}
	if calls != 0 {
