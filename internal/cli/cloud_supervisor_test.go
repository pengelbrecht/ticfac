...
// The command tests for `ticfac cloud supervisor`, ported from ticks'
// cmd/tk/cmd/cloud_supervisor_test.go: the factory harness is the one in
// cloud_test.go, the Workflows API is answered through a transport rather than
// a listener, and ExecuteArgs-with-captured-output becomes
...
func TestCloudSupervisorNamesTheStepALiveRunIsOn(t *testing.T) {
	...
	code, out, stderr := runCloudArgs(t, []string{"cloud", "supervisor", "run_live"})
	if code != exitSuccess {
		t.Fatalf("cloud supervisor: %s\n%s", stderr.String(), out.String())
	}
	output := out.String()
...
func TestCloudSupervisorSaysHowToInstallTheCredential(t *testing.T) {
	...
	code, _, stderr := runCloudArgs(t, []string{"cloud", "supervisor", "run_nocred"})
	if code == exitSuccess {
		t.Fatal("a factory with no Cloudflare API token answered anyway")
	}
	if !strings.Contains(stderr.String(), "--cloudflare-api-token") {
		t.Errorf("the refusal does not say how to install the credential: %s", stderr.String())
	}
}
...
func TestCloudSupervisorPrintsTheStepTrail(t *testing.T) {
	...
	code, out, stderr := runCloudArgs(t, []string{"cloud", "supervisor", "run_7n7", "--steps", "3"})
	if code != exitSuccess {
		t.Fatalf("cloud supervisor --steps: %s\n%s", stderr.String(), out.String())
	}
	output := out.String()
