...
func factoryCommand(ctx context.Context, args []string, stdout, stderr io.Writer) int {
	...
	case "deploy", "setup":
		// The deploy path is Factory move B (ticks tick b3a); this refusal is
		// what stands in for it until that tick's wiring lands, so an operator
		// is told which build will serve the command rather than getting a
		// bare "unknown subcommand".
		fmt.Fprintf(stderr, "ticfac factory %s is not wired in this build — it lands with the deploy path (Factory move B, ticks tick b3a)\n", name)
		return exitGeneric
	case "help", "-h", "--help":
		fmt.Fprint(stdout, factoryUsage)
		return exitSuccess
