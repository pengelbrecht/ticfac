...
// repoRoot returns the root directory of the git repository, ported from
// ticks' cmd/tk/cmd/init.go with the same exit-code contract: "not in a git
// repository" is its own code (tk's 3), because an orchestrator branching on
// it must not retry it as a generic failure.
func cloudRepoRoot() (string, error) {
	dir, err := os.Getwd()
	if err != nil {
		return "", err
	}
