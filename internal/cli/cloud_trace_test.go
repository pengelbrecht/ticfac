...
// The command tests for `ticfac cloud trace` and the run-id resolution the
// run-scoped commands share, ported from ticks' cmd/tk/cmd/cloud_trace_test.go
// and cloud_runid_test.go: the gateway logs API is stood up in-process, ticks'
// ExecuteArgs-with-captured-output becomes Run(args, stdout, stderr) with the
// exit code asserted, and the resolution-note test reads the two streams
// straight off Run instead of cobra's captured pair.
...
func TestCloudTraceReconstructsTheConversationFromRequestBodies(t *testing.T) {
	...
	code, out, stderr := runCloudArgs(t, []string{"cloud", "trace", "run_62c289d1e57942cea5fef6c1a508a0fd"})
	if code != exitSuccess {
		t.Fatalf("cloud trace: %s\n%s", stderr.String(), out.String())
	}
	output := out.String()
...
func TestCloudTraceCacheViewShowsPerCallHitRates(t *testing.T) {
	...
	code, out, stderr := runCloudArgs(t, []string{"cloud", "trace", "run_62c289d1e57942cea5fef6c1a508a0fd", "--cache"})
	if code != exitSuccess {
		t.Fatalf("cloud trace --cache: %s\n%s", stderr.String(), out.String())
	}
	output := out.String()
...
func TestCloudTraceToolsListsOnlyTheToolCalls(t *testing.T) {
	...
	code, out, stderr := runCloudArgs(t, []string{"cloud", "trace", "run_62c289d1e57942cea5fef6c1a508a0fd", "--tools"})
	if code != exitSuccess {
		t.Fatalf("cloud trace --tools: %s\n%s", stderr.String(), out.String())
	}
	output := out.String()
...
func TestCloudTraceCallDumpsOneExchangeInFull(t *testing.T) {
	...
	code, out, stderr := runCloudArgs(t, []string{"cloud", "trace", "run_62c289d1e57942cea5fef6c1a508a0fd", "--call", "2"})
	if code != exitSuccess {
		t.Fatalf("cloud trace --call 2: %s\n%s", stderr.String(), out.String())
	}
	output := out.String()
...
func TestCloudTraceRefusesACallNumberThatDoesNotExist(t *testing.T) {
	configureTraceGateway(t)
	traceRunGateway(t)
	code, _, stderr := runCloudArgs(t, []string{"cloud", "trace", "run_62c289d1e57942cea5fef6c1a508a0fd", "--call", "9"})
	if code == exitSuccess || !strings.Contains(stderr.String(), "no call 9") {
		t.Fatalf("an out-of-range --call is not refused clearly: %s", stderr.String())
	}
}
...
func TestCloudTraceJSONEmitsTheRawGatewayRows(t *testing.T) {
	...
	code, out, stderr := runCloudArgs(t, []string{"cloud", "trace", "run_62c289d1e57942cea5fef6c1a508a0fd", "--json"})
	if code != exitSuccess {
...
func TestCloudTraceSaysWhenNoCallsCarryTheRunID(t *testing.T) {
	...
	code, out, stderr := runCloudArgs(t, []string{"cloud", "trace", "run_unknown"})
	if code != exitSuccess {
		t.Fatalf("cloud trace: %s", stderr.String())
	}
	if !strings.Contains(out.String(), "No AI Gateway calls") || !strings.Contains(out.String(), "made no model call") {
...
func TestCloudTraceWithoutCostTelemetryNamesItsOwnRemedy(t *testing.T) {
	...
	code, _, stderr := runCloudArgs(t, []string{"cloud", "trace", "run_62c289d1e57942cea5fef6c1a508a0fd"})
	if code == exitSuccess || !strings.Contains(stderr.String(), "--cloudflare-api-token") {
		t.Fatalf("a missing telemetry token does not name its remedy: %s", stderr.String())
	}
}

func TestCloudTraceRefusesTwoViewsAtOnce(t *testing.T) {
	configureTraceGateway(t)
	code, _, stderr := runCloudArgs(t, []string{"cloud", "trace", "run_62c289d1e57942cea5fef6c1a508a0fd", "--tools", "--cache"})
	if code == exitSuccess || !strings.Contains(stderr.String(), "one at a time") {
		t.Fatalf("two views at once are not refused: %s", stderr.String())
	}
}
...
func TestCloudTraceFiltersOnTheRunIDMetadataPair(t *testing.T) {
	configureTraceGateway(t)
	seen := traceRunGateway(t)
	code, _, stderr := runCloudArgs(t, []string{"cloud", "trace", "run_62c289d1e57942cea5fef6c1a508a0fd", "--cache"})
	if code != exitSuccess {
		t.Fatalf("cloud trace: %s", stderr.String())
	}
...
func TestCloudTraceStatesTheRunsTraceID(t *testing.T) {
	...
	if !strings.Contains(out.String(), "trace: tr_0123456789abcdef0123456789abcdef") {
